		...
		if ( $args['type'] == 'donations' ) {
			...
			if ( $args['form'] == null ) {
				...
				if ( get_post_type( $args['form'] ) == 'give_forms' ) {
					...
					$error['error'] = sprintf( __( 'Donation form "%s" not found!', 'give' ), $args['form'] );
				}
			}
	...
	public function get_recent_donations() {
		...
		if ( $query ) {
			...
			foreach ( $query as $payment ) {
				...
				$sales['donations'][ $i ]['donations']      = array();

				$c = 0;

				//Loop through donations 
				foreach ( $payment->payment_details as $key => $item ) {

					$form_id  = isset( $item['id'] ) ? $item['id'] : 0;
					...
					if ( isset( $item['options']['price_id'] ) ) {
						$price_name = give_get_price_option_name( $form_id, $item['options']['price_id'], $payment->ID );
					}

					$sales['donations'][ $c ]['form']['id']         = $form_id;
					$sales['donations'][ $c ]['form']['name']       = get_the_title( $form_id );
					$sales['donations'][ $c ]['form']['price']      = $price;
					$sales['donations'][ $c ]['form']['price_name'] = $price_name;
					$sales['donations'][ $c ]['form']['price_id']   = $price_id;
